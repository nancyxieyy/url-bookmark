...
收藏成功后不再跳转到编辑页。AI 推荐和标签确认直接出现在首页，避免“刚输入网址就离开收藏列表”的割裂感；详情页只保留给之后查看正文和编辑元数据。
...
- 集成测试验证新增、首页标签确认、搜索、标签、编辑、软删除、恢复、彻底删除和过期清理。
...
- 使用无头 Chromium 验证页面渲染、首页删除确认、回收站恢复和彻底删除，并检查浏览器错误。
