...
正文抓取成功后，Web 端会继续停留在首页，并把标题、已有标签列表和 Markdown 前 10,000 字发送给 LLM，请它推荐标签。推荐结果不会自动写入数据库，而是分成“优先匹配已有标签”和“建议的新标签”展示；用户在首页点击建议并保存后才会入库。服务端会再次校验模型输出，总推荐数最多 5 个，其中新标签最多 2 个。这个功能遵循：
